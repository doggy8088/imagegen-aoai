...
These prompting principles apply to the bundled `scripts/image_gen.js` CLI workflow.

This file is about prompt structure, specificity, and iteration. CLI execution controls such as `quality`, `input_fidelity`, masks, output format, and output paths live in the CLI docs.
...
- In CLI mode, use `medium` or `high` quality for small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes.
...
## Transparent images
- Use the default `gpt-image-2` chroma-key workflow first for transparent-image requests. If the subject is clearly too complex for chroma-key removal, explain the `gpt-image-1.5` true-transparency path and ask before switching.
...
- After generation, remove the background locally with `node "${CODEX_HOME:-$HOME/.codex}/skills/.system/imagegen/scripts/remove_chroma_key.js" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill` and validate the alpha result before shipping it.
...
- Use CLI `gpt-image-1.5 --background transparent --output-format png` only after the user explicitly confirms that path, or when the user already explicitly requested `gpt-image-1.5` or `scripts/image_gen.js`. Ask first for true/native transparency requests, failed chroma-key validation, or complex transparent subjects such as hair, fur, glass, smoke, liquids, translucent materials, reflective objects, or soft shadows.

## CLI execution controls
- `quality`, `input_fidelity`, explicit masks, output format, and output paths are CLI execution controls.
- See `references/cli.md` and `references/image-api.md` for those controls.
- In CLI mode, `gpt-image-2` is the default. It supports `quality=low|medium|high|auto`; use `low` for fast drafts and thumbnails, and move to `medium`, `high`, or `auto` for final assets.
- `gpt-image-2` always uses high fidelity for image inputs, so do not set `input_fidelity` with that model.
- If a transparent request needs true CLI transparency, ask before using `gpt-image-1.5` unless the user already explicitly chose it. Explain that chroma-key removal is the default path, but `gpt-image-2` does not support `background=transparent`.
