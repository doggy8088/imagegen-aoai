#!/usr/bin/env python3
"""Fallback CLI for explicit image generation or editing with Azure OpenAI GPT Image models.
...
Defaults to an Azure OpenAI image deployment and a structured prompt augmentation workflow.
"""
...
def _default_model() -> str:
    return os.getenv("AZURE_OPENAI_IMAGE_DEPLOYMENT") or DEFAULT_MODEL
...
def _ensure_api_key(dry_run: bool) -> None:
    ...
    if not missing:
        print(
            f"Azure OpenAI environment is set (api-version {_azure_api_version()}).",
            file=sys.stderr,
        )
        return
    if dry_run:
        _warn(f"{', '.join(missing)} not set; dry-run only.")
        return
    _die(f"{', '.join(missing)} not set. Export them before running.")
...
def _validate_size(size: str, model: str) -> None:
    ...
    if not model.startswith(GPT_IMAGE_MODEL_PREFIX):
        if size == "auto" or _parse_size(size) is not None:
            return
        _die("size must be auto or WIDTHxHEIGHT, for example 1024x1024.")
...
def _validate_model(model: str) -> None:
    if not model.strip():
        _die("model must be an Azure OpenAI image deployment name.")
...
def _create_async_client():
    try:
        ...
        _die(
            "AsyncAzureOpenAI not available in this openai SDK version. "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fallback CLI for explicit image generation or editing via Azure OpenAI GPT Image deployments"
    )
